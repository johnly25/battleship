...
test('check if there is a ship infront when', () => {
    const gameboard = new Gameboard(10);
    gameboard.placeShip([1, 0], 2, false);
    expect(gameboard.checkCollide([0,0], 2, false)).toBe(true);
}) 

test('check if there is a ship infront when', () => {
    const gameboard = new Gameboard(10);
    gameboard.placeShip([2, 0], 2, false);
    expect(gameboard.checkCollide([0,0], 2, false)).toBe(false);
}) 

test('cannot place a length 2 ship in front of ship thats already placed', () => {
    const gameboard = new Gameboard(10);
    gameboard.placeShip([1,0], 2, false);
    console.log(gameboard);
    expect(gameboard.placeShip([0,0], 2, false)).toBe(false);
